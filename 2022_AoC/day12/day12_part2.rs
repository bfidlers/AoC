...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let start: (usize, usize) = find_elems(&data, 'S' as i32 - 'a' as i32)[0];
    let end: (usize, usize) = find_elems(&data, 'E' as i32 - 'a' as i32)[0];
    data[start.1][start.0] = 'a' as i32 - 'a' as i32;
    data[end.1][end.0] = 'z' as i32 - 'a' as i32;

    let starts: Vec<(usize, usize)> = find_elems(&data, 0);
    let mut dists: Vec<Option<i32>> = Vec::new();
    for s in starts {
        dists.push(a_star(&s, &end, &data));
    }

    let result: Option<i32> = *dists.iter()
                                    .filter(|x| x.is_some())
                                    .min()
                                    .unwrap();
...
fn find_elems(matrix: &Vec<Vec<i32>>, elem: i32) -> Vec<(usize, usize)> {
