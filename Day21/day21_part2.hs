...
parseInput input = intercalate "," . map (\ x -> getMatch x) $ sortedAllergens
    where sortedAllergens = sortOn (\ x -> getName x) matchedAllergens
          matchedAllergens = findSolution newAllergens
