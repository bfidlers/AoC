...
main = do input <- readFile "input.txt"
          print . (+(-1)) .countMultipleBags . filter (isName "shinygold") . fillInDataStructure . parseInput $ input
...
fillIn bag1 (Bag nb name bags bool)
  | bagName bag1 == name = Bag nb (bagName bag1) (bagSubBags bag1) True
  | hasName (bagName bag1) (Bag nb name bags bool) = Bag nb name (map (fillIn bag1) bags) bool
  | otherwise = (Bag nb name bags bool)

countBags :: Bag -> Int
countBags (Bag nb _ bags _) = nb + nb * countMultipleBags bags
countBags Empty = 0
countBags Unknown = 0

countMultipleBags :: [Bag] -> Int
countMultipleBags [] = 0
countMultipleBags (x:xs) = countBags x + countMultipleBags xs
